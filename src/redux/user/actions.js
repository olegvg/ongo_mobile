...
export function updateCategoryFilter(selIndex, selState) {
  if (Firebase === null) {
    return () => new Promise((resolve, reject) =>
      reject({ message: ErrorMessages.invalidFirebase }));
  }
  ...
  const ref = FirebaseRef.child(`users/${UID}/invertedCatFilter`);

  if (selState === false) {
    ref.child(selIndex).set(false);
  } else {
    ref.child(selIndex).remove();
  }
  return async () => {};
}
